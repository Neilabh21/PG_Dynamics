...
%positon of bob in the base frame
l=1/n;
x=zeros(size(YSol,1));
y=zeros(size(YSol,1));
...
for i=1:(size(YSol,2)/2)
    x(j)=x(j)+l*sin(rad2deg(YSol(j,i)));
    y(j)=y(j)+l*cos(rad2deg(YSol(j,i)));
end
end
...
b = zeros(n_size, 1);
b(1) = - k / (m * l ^ 2) * (2 * Y(1) - 0 - Y(2)) +  g / l * sin(Y(1)) + cos(Y(1)) * (val_1) - sin(Y(1)) * (val_2);
for i = 2:n_size-1
    b(i) = - k / (m * l ^ 2) * (2 * Y(i) - Y(i-1) - Y(i+1)) +  g / l * sin(Y(i)) + cos(Y(i)) * (val_1) - sin(Y(i)) * (val_2);
end
b(n_size) = - k / (m * l ^ 2) * ( Y(n_size) - Y(n_size-1)) +  g / l * sin(Y(n_size));
